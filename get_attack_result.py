...
def attack_parse_args():
    ...
    parser.add_argument("--model_name",
                        default='/root/save_models/baselines/fine-tune_20220326/finetune_bert-base-uncased_glue-sst2_lr2e-05_epochs3_seed42/epoch2',
                        type=str)
    ...
    parser.add_argument("--perturbed_file", default='bert_sst2-train_only-attack.csv', type=str)
...
def main():
    ...
    if args.save_perturbed:
        with open(args.perturbed_file, 'w', encoding='utf-8', newline="") as f:
    ...
    for result in attacker.attack_dataset():
        ...
        if args.save_perturbed and result.original_result.output == result.original_result.ground_truth_output:

            with open(args.perturbed_file, 'a', encoding='utf-8', newline="") as f:
                # csv_writer = csv.writer(f, delimiter='\t')
                csv_writer = csv.writer(f)
                csv_writer.writerow([result.perturbed_result.attacked_text.text, result.perturbed_result.output])
    ...
    csv_writer.writerow([args.model_name, original_accuracy, accuracy_under_attack, attack_succ])
    out_csv.close()
